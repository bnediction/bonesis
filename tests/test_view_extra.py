...
class testExtraView(unittest.TestCase):
    def setUp(self):
        pkn = bonesis.InfluenceGraph.complete(
            "abc", sign=1, loops=False, allow_skipping_nodes=False
        )
        data = {
            "x": {"a": 0, "b": 0},
            "y": {"a": 1, "b": 1},
    ...
    def test_extra_configurations(self):
        for view in [self.bo1.boolean_networks, self.bo1.diverse_boolean_networks]:
            c = 2
